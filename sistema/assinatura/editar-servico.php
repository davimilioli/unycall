...
$dadosServico = $gerenciador->consultarDadosServico($_GET['edit']);
...
if (isset($_POST['tipo'], $_POST['nome'], $_POST['disp_regiao'], $_POST['preco'], $_POST['status'])) {
    ...
    $editarServico = array(
        ...
        'custo' => str_replace(',', '.', $preco),
        'status' => $status
    );

    $gerenciador->atualizarDadosServico($editarServico);
                ...
                <form method="POST" class="form">
                    <div class="form-container register-user">
                        <div class="form-category">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="tipo" id="tipo" required value="<?= $dadosServico['tipo'] ?>">
                            </div>
                            <div class="form-group">
                                <label for="nome">Nome <span>*</span></label>
                                <input type="text" name="nome" id="nome" required value="<?= $dadosServico['nome'] ?>">
                            </div>
                            <div class="form-group">
                                <label for="disp_regiao">Disponibilidade de região <span>*</span></label>
                                <input type="text" name="disp_regiao" id="disp_regiao" required value="<?= $dadosServico['disp_regiao'] ?>">
                            </div>
                            <div class="form-group">
                                <label for="preco">Preço <span>*</span></label>
                                <input type="text" name="preco" id="preco" required value="<?= $dadosServico['custo'] ?>">
                            </div>
                        </div>
                        <div class="form-category">
                            <h2>Status</h2>
                            <div class="form-group">
                                ...
                                <select name="status" required>
                                    <option value="1" <?= $dadosServico['status'] == 1 ? 'selected' : '' ?>>Ativo</option>
                                    <option value="0" <?= $dadosServico['status'] == 0 ? 'selected' : '' ?>>Inativo</option>
                                </select>
                            </div>
                        </div>
                    </div>
                    <div class="form-buttons">
                        <div class="form-actions">
                            ...
                            <?php endif ?>
                            <div class="form-buttons">
                                <a class="btn secondary" id="excluirUsuario" data-id-adm="<?= $id ?>" data-id="<?= $dadosServico['id'] ?>">Excluir</a>
